...
#if defined(ESP32)
    #define PIN_LED                5 // Message-LED
    #define PIN_SEND               11
    #define PIN_RECEIVE            14
#else
...
void getFunctions(bool *ms,bool *mu,bool *mc);
    ...
    WiFiClient myWifiClient; //client that is connected
...
void setup() {
  ...
  // Attach cronjob function to our timer.
  ...
  if (!EEPROM.begin(EEPROM_SIZE)){
    Serial.println("EEPROM init for ESP32 failed!");
  }else
    Serial.println("EEPROM init for ESP32 OK.");
#else
...
#if defined(ESP32)
  if(myWifiClient){
      if(myWifiClient.available()){
        char inChar = (char)myWifiClient.read();
        switch(inChar)
        {
          case '\n':
          case '\r':
      ...
      myWifiClient=wifiServer.available();
  }
#endif //ESP32
...
void send_raw(const uint8_t startpos,const uint16_t endpos,const int16_t *buckets, String *source=&cmdstring)
{
	...
	for (uint16_t i=startpos;i<=endpos;i++ )
	{
		...
		isLow ? digitalWrite(PIN_SEND,LOW) : digitalWrite(PIN_SEND,HIGH);
		stoptime+=dur;
	}
	while (stoptime > micros()){
		;
		...
		for (bit = 0x8; bit>0; bit >>= 1) {
			for (byte i = 0; i <= 1; i++) {
				if ((i == 0 ? (b & bit) : !(b & bit)))
					...
					digitalWrite(PIN_SEND,HIGH);//digitalHigh(PIN_SEND);
...
void send_cmd()
{
	...
	for (uint8_t i=0;i<repeats;i++)
	{
		for (uint8_t c=0;c<=cmdNo;c++)
		{
			...
			digitalWrite(PIN_SEND,LOW);//digitalLow(PIN_SEND);
		}
...
#endif
{
  ...
  #define  cmd_uptime 't'
  #define  cmd_changeReceiver 'X' //XE enable receive, XQ disable receive
  #define  cmd_space ' '
  #define  cmd_help '?'
  #define  cmd_changeFilter 'F'
  #define  cmd_send 'S'
  #define  cmd_ping 'P'
  #define  cmd_config 'C'  //CG show config, C[E|D][SUC] Enable|Disable [MS|MU|MC] protocols
...
void changeReciver() {
  ...
  {
  	disableReceive();
    MSG_PRINTLN("Receive disabled");
  }
  if (cmdstring.charAt(1) == 'E')
  {
  	enableReceive();
    MSG_PRINTLN("Receive enabled");
  }
}
...
void getFunctions(bool *ms,bool *mu,bool *mc)
{
    int8_t dat = (int8_t) EEPROM.read(addr_features);
